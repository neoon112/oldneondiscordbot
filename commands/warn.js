...
module.exports = {
    ...
    async execute (message, args) {
        if(!message.member.hasPermission("MANAGE_SERVER")) return message.reply('❌ You can\'t use that');

        const user = message.mentions.users.first() || message.guild.members.cache.get(args[0]);

        if(!user) return message.reply('❌ Please specify a user, via mention.');
        ...
        if(message.guild.owner.id === user.id) return message.reply('❌ You can\'t warn the server\'s owner');
